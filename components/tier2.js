...
module.exports = function (tier2) {
        ...
        connection.query(sql_id, req.user, (err, foundId) => {
          ...
          connection.query(
            ...
            function (err, PayrollFound, fields) {
              ...
              let totalTier2 = 0;
              PayrollFound.forEach((tier2) => {
                const tier2Return = parseFloat(
                  tier2.Tier2_Returns.replace(",", "")
                );
                totalTier2 += tier2Return;
              });
              res.render("tier2", {
                page_name: "tier2",
  ...
  tier2.get("/tier2Returns", authenticated.checkAuthenticated, (req, res) => {
